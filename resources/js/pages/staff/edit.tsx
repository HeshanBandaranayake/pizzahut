...
import { Save, ArrowLeft, Mail, ShieldCheck, KeyRound, UserCog } from 'lucide-react';
...
export default function Edit({ member }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `Edit ${member.name}`, href: `/staff/${member.id}/edit` },
    ];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6 max-w-2xl mx-auto">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black italic tracking-tight text-[#EE1922]">EDIT TEAM MEMBER</h2>
                        <p className="text-sm text-muted-foreground font-medium italic uppercase tracking-tighter">Update Profile for {member.name}</p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="grid gap-6">
                    <Card className="border-sidebar-border/70 shadow-sm">
                        <CardHeader className="pb-4">
                            <CardTitle className="text-sm font-black italic uppercase tracking-widest text-muted-foreground">Account Details</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-muted/20">
                                <div className="col-span-full">
                                    <Label className="font-black italic uppercase text-[11px] text-[#EE1922]">Reset Password (Optional)</Label>
                                    <p className="text-[10px] text-muted-foreground mb-4">Leave both fields empty if you don't want to change the password.</p>
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="password" className="font-bold uppercase text-[10px] tracking-widest">New Password</Label>
                                    <div className="relative">
                                ...
                                <div className="grid gap-2">
                                    <Label htmlFor="password_confirmation" className="font-bold uppercase text-[10px] tracking-widest">Confirm New Password</Label>
                                    <div className="relative">
                                        <KeyRound className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground/50" />
                                        <Input
                    ...
                    >
                        <Save className="mr-3 size-6" /> Update Member Profile
                    </Button>
                </form>
            </div>
        </AppLayout>
